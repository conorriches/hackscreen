  ...
  function($scope, $timeout, $filter, sharedService, socket) {
    ...
    $scope.showScreen = function(indexToShow) {
      ...
      $scope.transitionScreens(indexToShow, function() {
        $timeout(function() {
          var next =
            $scope.currentScreenIndex === $scope.screens.length - 1
              ? 0
              : $scope.currentScreenIndex + 1;
        ...
        switch (data.topic) {
          ...
          case "door/outer/state":
            if (data.message === "opened") {
              $scope.setDoor(true);
              $scope.doorLeftOpenPromise = $timeout(function() {}, 2000);
            }
          ...
          case "button/big/red/state":
            ...
            if (n < 4) {
              ...
              $scope.setNotification(
                "DO NOT DO THAT AGAIN FUCKO",
                NOTIF_DANGER
              );
            } else {
              $scope.setNotification(
                "This is Fiona Bruce. Shove it.",
                NOTIF_DANGER
              );
            }
...
app.controller("IndexCtrl", [
  ...
  "$scope",
  "mySharedService",
  function($rootScope, $scope, sharedService) {
